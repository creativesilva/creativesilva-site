...
def aperture_settings_section(es):
    ...
    if es:
        ...
        note_t=("Captura en <strong>RAW</strong> para tener el mayor detalle para editar. Empieza en f/8 y hazla un paso m&aacute;s brillante: apunta a <strong>+1</strong> en el expos&iacute;metro. "
        ...
        note_t=("Capture in <strong>RAW</strong> so you have the most detail to edit. Start at f/8 and make it one stop bright: aim for <strong>+1</strong> on the light meter. "
...
def overview():
    ...
    en+=type_card("overview","The Module Overview","One Partner, Three Apertures, Your Preset",
        para("This is a partner photo walk about <strong>aperture</strong>. You and a partner take turns: one is the <strong>photographer</strong> and the other is the <strong>talent</strong> (the person being photographed). Then you switch, so each of you photographs and each of you gets photographed.")
...
def step01():
    ...
    en+=card("YOUR THREE APERTURES / SET THE CAMERA","f/8, then f/4, then f/2",
        para("Keep ISO at 100 the whole time. Capture in RAW. Capture a good portrait at each aperture, and balance the light with your shutter each time.")
        + steps([
    ...
    es+=capture_format("raw", True)
    es+=type_card("photo-walk","Paso 01 &middot; En la Caminata","Tomen Turnos: Fot&oacute;grafo y Modelo",
        para("Salgan con tu compa&ntilde;ero y el kit de c&aacute;mara. Uno es el <strong>fot&oacute;grafo</strong> y el otro es el <strong>modelo</strong>. Captura tu serie completa, luego cambien de rol.")
    ...
    es+=card("TUS TRES APERTURAS / AJUSTA LA C&Aacute;MARA","f/8, luego f/4, luego f/2",
        para("Mant&eacute;n el ISO en 100 todo el tiempo. Captura en RAW. Captura un buen retrato en cada apertura y equilibra la luz con tu obturador cada vez.")
        + steps([
...
def step02():
    ...
    en+=type_card("edit","Step 02 &middot; Cull","Choose Your Three Best",
        ...
        + (float_right(S2_FLOAT,"A Pioneer Valley student editing a RAW portrait in Lightroom Classic on an iMac in the lab","Run your preset, then fine-tune it for these portraits.") if HAVE_IMAGES else float_ph("FLOAT-RIGHT PLACEHOLDER &middot; Step 2 edit photo. Swap in when ready."))
        + bullets([
    ...
    es+=type_card("edit","Paso 02 &middot; Selecciona","Elige Tus Tres Mejores",
        ...
        + (float_right(S2_FLOAT,"Un estudiante de Pioneer Valley editando un retrato RAW en Lightroom Classic en una iMac en el laboratorio","Corre tu preset, luego aj&uacute;stalo para estos retratos.") if HAVE_IMAGES else float_ph("PLACEHOLDER FLOTANTE &middot; foto del Paso 2"))
        + bullets([
