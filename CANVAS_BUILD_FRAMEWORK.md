...
## 3.5 Angular Gradient Framework (Finals / Study Guide style) — LOCKED

Approved master reference page: `curriculum/shared/da-finals-quiz-prep.html`.
Generator: `tools/build-da-finals.py` (run with `python3` to regenerate).
This is the house style for Finals study guides, quiz-prep, and test-intro
pages. Use it verbatim. Do not improvise new border or button treatments.

### Palette tokens
```
Teal   solid #00b8b8   dim rgba(0,184,184,0.08)   eyebrow #80e0e0   tint linear-gradient(180deg,rgba(0,116,116,0.10) 0%,rgba(0,116,116,0.03) 100%)
Orange solid #FF6B1A   dim rgba(255,107,26,0.08)  eyebrow #ffb27c   tint linear-gradient(180deg,rgba(255,107,26,0.16) 0%,rgba(255,107,26,0.04) 100%)
Cyan   solid #00c2ff   dim rgba(0,194,255,0.08)   eyebrow #7dd3fc   tint linear-gradient(180deg,rgba(0,194,255,0.16) 0%,rgba(0,194,255,0.04) 100%)
Red    solid #E62429   dim rgba(230,36,41,0.08)   eyebrow #ffb3b6
```

### Card borders — THE CRITICAL RULE
Cards stay **translucent** so the PV watermark shows through. The accent
frame is a diagonal gradient on the card itself; the bright solid top
stripe is a thin child div pulled flush to the top with negative margins.

DO NOT use a colored background wrapper to fake the frame — a bright
wrapper bleeds through the translucent card and floods it. DO NOT use
`border-image` slice `0 1 1 1` to keep a solid top — an active
border-image suppresses the solid `border-top-color`, so the top edge
renders invisible. Both were tried and both failed. The recipe below is
the one that works.

Outer section card (2px frame, 4px solid top stripe):
```html
...
Inner tile (1px frame, 3px solid top stripe):
```html
...
```
The stripe child's negative margins must equal the card's top/left/right
padding so it sits flush to the inner border edge. Inner-tile wedge
background (optional accent): `linear-gradient(135deg,<accent 0.14> 0%,<accent 0.14> 10%,rgba(0,0,0,0.32) 10%,rgba(0,0,0,0.32) 100%)`.

No corner triangles on this style. They were removed.

### Buttons — THE FRAMEWORK RULE
...
- The 2px top accent bar = the SOLID color of the section the button sits in.
  Toggle in the teal banner -> teal bar. Download in an orange section -> orange bar.
...
```
Hairline divider under each card title: `<div style="height:2px;background:<SOLID>;width:60px;margin-bottom:22px;"></div>` (32px wide on inner tiles).

### Chip numbering
Number the main narrative sections sequentially: `WELCOME / 01`,
`TEST FORMAT / 02`, `STUDY / 03`, ... and the closing/download card
continues that count. Grouped repeating sections get their own
`XX / total` series (e.g. categories run `CATEGORY 01 / 08` ... `08 / 08`).

### Bilingual mirror — STANDING RULE
All Canvas content is bilingual: full English block first, then a full
Spanish mirror below the `#espanol` anchor. English-only ONLY when the
user explicitly says so. Banner language toggle swaps
`Clic para Español` (EN side) / `Back to English` (ES side).
