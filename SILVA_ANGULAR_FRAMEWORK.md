...
**Download buttons are official orange + titled by their file (LOCKED 2026-09-03):** every button that downloads a FILE uses the official orange `#FF6B1A` (white bold text, `border-top:2px solid #ffb27c`) so it stands out for students. External read/reference links (an article, a webpage, `download=False`) keep the light cream style, so orange always means &#x201C;download a file.&#x201D; The download section&#x2019;s HEADING names exactly what is downloaded there (&#x201C;Download Reflection Document&#x201D;, not a generic &#x201C;Resources&#x201D;), and the section shows the `overview` folder (&#x201C;downloads&#x201D;) icon next to the orange button (a `dl_row`: 42px icon + button in an `align-items:center` flex row). Moving forward, name each download section for the file the student needs there. On a photo-walk module Overview, the top-right identification icon is `photo-walk` (&#x201C;In-Class Photo Walk&#x201D;), not `overview`; the downloads/folder icon lives down beside the reflection/file download instead.
