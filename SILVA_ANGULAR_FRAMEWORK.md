...
- Deliverables, per step (REQUIRED, no exception): each STEP is its own separately-graded Canvas submission with its own deliverable(s), always, unless Chris says otherwise (e.g. Sketchbook step 1 = upload 2 images, step 2 = upload the reflection: two graded assignments). The compact `DELIVERABLES` / `ENTREGABLES` box lives ONLY on the step page that submits that deliverable, placed at the **TOP of the step** (right after the banner, before the content cards, LOCKED 2026-09-10, same as the Downloads section on Overviews), stating exactly what THAT step turns in with a clear file type (.docx, JPG, high-resolution JPG, .xmp). The box is **GOLD-accented** (its own color so turn-ins are unmistakable, distinct from teal content, orange Downloads, and purple Resources): `background:rgba(245,179,1,0.12);border:1px solid rgba(245,179,1,0.35);border-left:5px solid #f5b301`, eyebrow `#ffd166` reading `DELIVERABLES &middot; TURN IT IN` / `ENTREGABLES &middot; ENTR&Eacute;GALO`, bullets `#f5b301`, with the gold deliverables turn-in icon (`deliverables-v4.png`, 44px) at the top-right of the title row. NEVER put deliverables on the Overview (the Overview describes the project and holds downloads). EN in `#top`, ES in `#espanol`.
- Assignment identification icon (REQUIRED where an icon exists): on every active module page the FIRST card&#x2019;s BIG TITLE row (the 20pt heading, e.g. &#x201C;Reflection&#x201D;) becomes a `space-between` flex row (`<!--IDEYE-->`). The small eyebrow/step line above it (e.g. &#x201C;STEP 03 / REFLECT&#x201D;) is left untouched. The heading stays on the LEFT; on the RIGHT sits a short orange description followed by the assignment icon (46px), so the icon lands at the far-right edge, sized to sit on the title line. The whole right group wraps below the title on narrow screens (`flex-wrap:wrap`). EN in `#top`, ES in `#espanol`. No standalone orange bar. Icon family in `assets/Icons/assignment/` (220px PNGs rendered from Chris&#x2019;s SVG masters in `assets/Icons/assignment/_src/`) with its right-side description: `overview` &rarr; &#x201C;Module Overview&#x201D; (all overviews, uniform); `your-device` &rarr; &#x201C;Equipment: Your Device&#x201D;; `camera-kit` &rarr; &#x201C;Equipment: Camera Kit&#x201D;; `photo-walk` &rarr; &#x201C;In-Class Photo Walk&#x201D;; `reflection` &rarr; &#x201C;Written Reflection&#x201D;; `design` &rarr; &#x201C;Design &amp; Create&#x201D;; `edit` &rarr; &#x201C;Cull &amp; Edit&#x201D;; `research` &rarr; &#x201C;Find &amp; Collect&#x201D;; `worksheet` &rarr; &#x201C;Worksheet&#x201D;; `sketch` &rarr; &#x201C;Sketch by Hand&#x201D;. Equipment-requirement icons use the &#x201C;Equipment: X&#x201D; label form (moving forward). The `downloads` folder icon is separate (download section, not a top-right type). Apply with the idempotent `tools/apply-id-icons.py` (add the page to its MAP with the type); it strips any legacy `<!--IDCHIP-->` bar, unwraps any prior `<!--IDEYE-->` row, and re-runs safely after regenerating any builder module. The family is complete: every active page maps to a type.
...
**Vocabulary / Key Words is a PURPLE Resource (LOCKED 2026-09-11).** Vocab is technically a reference resource, so the Key Words section is a `resources_card` titled &#x201C;Module Resource: Key Words&#x201D; / &#x201C;Recurso del M&oacute;dulo: Palabras Clave&#x201D; (purple chip + resources gear icon), and it stays on the Overview. Every accent inside it is purple (cohesion): the 6-term grid is a `<table>` (3 columns x 2 rows) with purple gradient frames over a purple-dark body wedge (`#241d3a` / `#140f24`), and the quiz-disclaimer note at the top is a PURPLE note box (`rgba(139,92,246,0.10)` bg, `#8b5cf6` left border, eyebrow `#c4b5fd` reading &#x201C;ON THE QUIZ&#x201D; / &#x201C;EN EL EXAMEN&#x201D;). Give every term box the SAME size for uniformity (`min-height:132px`) and write the 6 definitions at roughly the same length so no box towers over another. Definitions stay concise but clear, EN + ES at 5th grade.
