...
**Combined chip header = FULL-WIDTH gradient bar (UPDATED 2026-09-18/20).** Every section title is one **full-width bar** holding the section icon on the LEFT and one big color-coded title, with a short accent rule directly under it. The bar spans the card and its dark fill **gradients out to transparent on the right** (`linear-gradient(90deg,rgba(0,0,0,0.45) 0%,rgba(0,0,0,0.28) 45%,rgba(0,0,0,0) 100%)`), replacing the old compact `inline-flex` dark box. No separate eyebrow above, no repeated heading below.
```html
<div style="display:flex;align-items:center;gap:12px;background:linear-gradient(90deg,rgba(0,0,0,0.45) 0%,rgba(0,0,0,0.28) 45%,rgba(0,0,0,0) 100%);border-left:5px solid <ACCENT>;padding:9px 18px 9px 12px;margin-bottom:12px;box-sizing:border-box;">
...
**CTE Standards bar = the SAME gradient bar, wrapped in a collapsible (LOCKED 2026-09-20).** The green CTE Standards title (`standards_box`, accent `#26de78`) uses the identical full-width gradient bar as `section_header` (same 90deg gradient, same 62px height, same translucency, no extra opaque box), so it matches Download Your Files and the other bars, and it is **aligned** with them (the `<summary>` gets `padding:14px 30px` so the bar's left edge lands at the same 36px indent as a `section_header` inside a 30px-padded box). It is wrapped in a `<details>` for the tap-to-reveal standards list; the disclosure **arrow is a custom green triangle placed BETWEEN the icon and the title** (the `<summary>` is `display:block` so the browser's own left marker is suppressed inline), and the &#x201C;Tap to see...&#x201D; lead rides over the faded right of the gradient.
...
**Live Image Catalog (LOCKED 2026-09-11).** The teacher catalog keeps a visual inventory of every AI-generated header + float-right image, in Build Resources &rarr; Image Generation Tools &rarr; **Live Image Catalog** (a sub-accordion under Cast), grouped by course, in module order (first module, first page, onward), as Download / Copy / lightbox cards labeled by module and page/step. It is GENERATED by `tools/build-image-catalog.py`, which scans the live module pages: the **HEADER** is the first teal-content-framed image on each overview (the hero), and the **FLOATS** are the images in the `flex:1 1 44%` float column on every page. Module membership + order come from curriculum.html's own `MODULES` array, and it writes between the `<!-- LIVE_IMAGE_CATALOG_START/END -->` markers. **RE-RUN `tools/build-image-catalog.py` after adding a module or adding/replacing any header or float image** (it is part of finishing a module): because it only lists images the pages currently reference, a replaced image drops out and the new one appears automatically, so the catalog never shows an obsolete asset. It deliberately EXCLUDES tutorial screen captures, example photos, slide-panel images, and resource thumbnails (none of those use the overview hero or the float column).
...
**Deliverables box** = the gold chip header (icon on the LEFT), placed right AFTER the step's opening teal information section (NEVER leading the step, LOCKED 2026-09-24), opening with a forecast line, then the turn-in bullets. Helper `deliverables_box(es, items)`. Forecast:
...
**Slide-deck master storage (LOCKED 2026-09-11, PROCEDURE).** Whenever a module or step has a slide deck, BOTH the editable original (`.pptx`) AND the exported `.pdf` must be kept for future use, in two places: (1) the master archive `assets/slide-decks/<Course>/` (per-course subfolder), and (2) hosted in `assets/course-documents/` so the catalog + module can link them. Then add the deck to `SLIDE_DECKS` in `curriculum.html` as `{ label, pptx, pdf }` (per language when the deck is bilingual), so the teacher catalog's per-course **Slide Decks** section offers BOTH downloads. A `null` pptx or pdf renders as an orange "PPTX pending" / "PDF pending" chip, the signal to go find and place that file. So every module deck should end with a matched pptx + pdf pair in both locations and a two-download row in the catalog. **When only the `.pptx` exists, do NOT leave the PDF pending: export it yourself** with `soffice --headless --convert-to pdf --outdir <dir> <deck>.pptx` (a slide deck exports one PDF page per slide with no trailing blank, so no trimming needed, unlike Word syllabi). Only a genuinely absent editable master (a deck that has only a PDF, no source `.pptx`/Keynote anywhere) stays pending until Chris supplies the original.
...
div.silva-page > div#silva-module-content
  div#top  (this is the Canvas copy root: page background + watermark)
    ...
    details.silva-langbar  SPANISH mirror in a COLLAPSIBLE language accordion (starts CLOSED);
      the #espanol anchor is on the content div INSIDE it; the accordion uses flex column-reverse
      so its toggle STRIP sits at the BOTTOM of the open Spanish (and right after the English when
      collapsed). The banner + section cards inside are the same, translated.
  copy/download + language-toggle <script>
  <script src="/js/silva-nav.js"></script>
```

Banner: a dark teal panel with a **thick teal left rule** and teal top/bottom borders, PV logo left, centered eyebrow + title + hairline + tagline, language toggle button on the right. The single document download button lives at the END of the page in the closing card, not in the banner.

**Banner is a BALANCED 3-zone bar (LOCKED 2026-09-18).** Three flex zones: **logo LEFT**, **title block CENTER**, **controls RIGHT**. The left and right zones use EQUAL flex (`flex:1 1 120px` each) and the center grows hard (`flex:100 1 260px`), so the title block is TRULY centered relative to the whole bar, not pushed off-center by a wider control cluster (verify: the title's center-x equals the bar's center-x). Controls = a SHORT language toggle with the white module icon STACKED ABOVE it (`flex-direction:column`). The toggle text is derived from link direction inside `banner()` (`es_href=="#espanol"` &rarr; &#x201C;Espa&ntilde;ol&#x201D;, else &#x201C;English&#x201D;), NOT the long &#x201C;Clic para Espa&ntilde;ol&#x201D; / &#x201C;Back to English&#x201D;, so the cluster stays small and every module matches. The white module icon (`hicon`) is per-module: `HICON_YOUR_DEVICE` (own-device), `HICON_PHOTO_WALK` (camera-kit); a module with no type icon shows just the toggle. Canvas-safe (inline flex, no CSS grid / `@media`); the row wraps cleanly on phones. All builders delegate to the shared `banner()`.

**Per-page banner TITLES must differ per page (LOCKED 2026-09-18).** The big banner title (2nd arg of `banner()`) is step-specific, never repeated across a module&rsquo;s pages. Convention: OVERVIEW = eyebrow `Course &bull; Module` + title = the module name; each STEP = eyebrow `Module &bull; Step N` + a short action title (&#x201C;Capture &amp; Submit&#x201D;, &#x201C;Reflection&#x201D;, &#x201C;Design Your Cover&#x201D;, &#x201C;Brand Color Analysis&#x201D;). Repeating one title across all pages is a defect.

**Banner BACKGROUND is a dark cohesive teal panel (LOCKED 2026-09-20).** The banner fill is a dark symmetric teal gradient (`background-color:#04191a` under `linear-gradient(90deg,rgba(3,18,20,0.95) 0%,rgba(0,66,68,0.5) 50%,rgba(3,18,20,0.95) 100%)`: dark edges, a teal glow through the center) with a **2px teal top and bottom border** and a **7px teal left rule** (`border-left:7px solid #00b8b8`), so the header blends with the page's dark-teal aesthetic. Do NOT put a panther watermark in the banner: the watermark asset is the full PV crest, so it reads as a duplicate of the left logo (the faint panther behind the page content is the page background, `top_wrap`, which is enough). This replaced the older bright `135deg` black-to-`#007474` diagonal. The balanced 3-zone layout above is unchanged. A held ALTERNATIVE (lighter left-to-right gradient, more separation) exists but is NOT built unless Chris asks.

**Spanish LANGUAGE ACCORDION (LOCKED 2026-09-20).** The Spanish mirror is tucked into a collapsible `<details class="silva-langbar">` (helper `lang_accordion(es)` in `top_wrap`), CLOSED by default, so the page reads at a normal length and (when pasted) Canvas&rsquo;s submit button sits close instead of below the whole Spanish copy. The `<details>` uses inline `display:flex;flex-direction:column-reverse`, so the toggle **STRIP** (the `<summary>`) sits at the BOTTOM of the open Spanish and right after the English content when collapsed. The strip carries: an `ESPA&Ntilde;OL` chip, a green disclosure arrow **to the RIGHT of the chip**, the &#x201C;Ver esta tarea en espa&ntilde;ol&#x201D; title, and an **ENGLISH** button on the right. Tapping the strip toggles it (native `<summary>` tap, which is the ONLY thing that collapses a `<details>` in pasted Canvas, so this works when pasted). Both banners keep a language button: the EN banner &#x201C;Espa&ntilde;ol&#x201D; targets `#espanol` (the content div inside the accordion) so a jump auto-opens it (works in Canvas); the ES banner &#x201C;English&#x201D; targets `#top`. A small script in `wrap_page` (outside `#top`, so live-page only) drives smooth scroll off the accordion&rsquo;s `toggle` event: open &rarr; scroll to the Spanish, close &rarr; scroll to top. `#espanol` moved from the old `<div>` onto the accordion&rsquo;s content div. NOTE: the mutually-exclusive `<details name>` grouping does NOT survive Canvas paste, and text/label swaps on open/close need JS (live/linked only), so in pasted Canvas the strip keeps one fixed label.
...
**Contact-sheet templates: do NOT bundle them in a module (LOCKED 2026-09-24, Chris).** Once the presets are installed in Lightroom Classic they persist until deleted, so students already have them by now. Do NOT put the contact-sheet templates ZIP in a module's Downloads. If anyone ever needs them again they live on the **Course Overview**; a module's Downloads carries only that module's own files (e.g. its reflection doc). Contact sheets themselves are always turned in as high-resolution JPGs, never PDF (the preset prints to file with JPG preselected). (The old "install pointer under the ZIP row" rule only applied where the ZIP was offered; modules no longer offer it.)
...
**Person-being-photographed = "subject", never "talent"/"model" (LOCKED 2026-09-24, Chris).** In any partner/portrait module, the person in front of the camera is the **subject** (EN) / **sujeto** (ES), never "talent" or "model" (EN) or "modelo"/"talento" (ES). Those words frame the role as a performance and make camera-shy students feel they must perform. "Subject" is neutral: it just means the person being photographed. Applies to titles, body copy, and captions across all modules, EN + ES. Keep the pairing "Photographer and Subject" / "Fot&oacute;grafo y Sujeto." (Do not add any explanation about shyness or performing to the student copy; just use the neutral word.)

**Vocabulary / Key Words is a PURPLE Resource (LOCKED 2026-09-11).** Vocab is technically a reference resource, so the Key Words section is a `resources_card` titled &#x201C;Module Resource: Key Words&#x201D; / &#x201C;Recurso del M&oacute;dulo: Palabras Clave&#x201D; (purple chip + resources gear icon), and it stays on the Overview. Every accent inside it is purple (cohesion). **The terms are a COLLAPSIBLE accordion in TWO columns (LOCKED 2026-09-11):** each term is a native `<details>` (purple gradient frame over a purple-dark body wedge `#241d3a` / `#140f24`) whose `<summary>` shows a number + the term; tapping it reveals the definition. The terms sit in a `display:flex;flex-wrap:wrap;gap:10px;align-items:flex-start;` row with each `<details>` at `flex:1 1 45%;min-width:260px;`, so there are TWO columns on iPad/desktop and they collapse to one column only when the page is too narrow. No JS (native `<details>`), so it works either way. NOTE (corrected 2026-09-19): `<details>` accordions SURVIVE the Canvas paste (Chris live-tested), so this interactivity works whether the page is pasted or linked; only JS/`<style>`/`<details name>`-dependent behavior is live/linked only. Above the accordion sits the PURPLE quiz-disclaimer note (`rgba(139,92,246,0.10)` bg, `#8b5cf6` left border, eyebrow `#c4b5fd` reading &#x201C;ON THE QUIZ&#x201D; / &#x201C;EN EL EXAMEN&#x201D;) and a one-line &#x201C;Tap a word to open its meaning.&#x201D; / &#x201C;Toca una palabra para abrir su significado.&#x201D; hint. Definitions stay concise but clear, EN + ES at 5th grade. (Reference: Composition Concepts.)

**Content-photo float = `.silva-cfloat`, and it drops BELOW the text on narrow, never above (LOCKED 2026-09-18).** A content photo placed with `float_right()` is hoisted by the card into a `<div class="silva-cfloat" ...>` that is a TRUE right float at ~half width (`float:right;width:44%;min-width:280px;max-width:520px`): body text wraps beside it and then fills the FULL card width once past the image bottom (magazine wrap), instead of being trapped in a narrow flex column. The float styles are INLINE (not class-only) so a stale/missing stylesheet can never collapse it to a full-width block. On narrow screens (`@media max-width:640px`) the card (tagged `.silva-floatcard`) becomes `display:flex;flex-direction:column` and the image is `order:1`, so it falls BELOW the text (the float div sits first in source for the desktop wrap, so without this it would stack above). `note()`/`note_orange()` carry `overflow:hidden` (own BFC) so bordered callouts never slide under the float. This replaces the old flex two-column float AND the raw `float:right;width:4x%` inline pattern (the §8.5 placeholder markup is a placeholder only; real content photos use `float_right()` &rarr; `.silva-cfloat`). Do not revert to either old pattern.

**Camera settings: `camera_settings_section` (camera-kit) vs `own_device_capture` (own-device) (LOCKED 2026-09-18).** Camera-KIT modules (photo walk with the Canon kit) use `camera_settings_section(es, quality)`: a red two-column box with the on-screen settings mimic (the student adjusts only the shutter). OWN-DEVICE modules (phone / iPad) have no kit to dial in, so they use `own_device_capture(es)` instead: a full-width, NON-expandable red box styled like the Camera Settings section (red aperture icon + &#x201C;Camera Settings&#x201D; title + the red underline) whose verbiage says use your own phone/iPad at highest photo quality, capture in JPG, with the JPG badge inline. It replaces the `capture_format` chip on own-device capture steps. Verbiage says &#x201C;capture&#x201D;, never &#x201C;shoot&#x201D; (see Non-Negotiable Rules). **On camera-KIT steps, the RAW/JPG format is shown ONLY inside `camera_settings_section` (its Image Quality cell); do NOT add a standalone `capture_format` chip on the step (Chris, LOCKED 2026-09-24). The `capture_format()` helper is retired for standalone use.**
...
## 8.5 Image Placeholders (build-time only, NEVER pushed live)

**Do NOT ship placeholder boxes to the live site (LOCKED 2026-09-24, Chris: "if you don't have an image yet please remove placeholders for now").** A dashed/labeled placeholder box is a planning aid for local preview only. On anything pushed live, a slot whose real image is not supplied yet renders **nothing** (the card is just its text, full width): no placeholder box, no gap, no broken `<img>`. Gate each image on its own per-slot flag (e.g. `HAVE_S1_FLOAT`, `HAVE_HEADER`) whose `else` branch is `""`, so the image appears the moment it is placed and the flag flips, with no other edit. This is how the Aperture Portrait builders work (`tools/build-aperture-portrait*.py`): header + reflection floats are live, Step 1/2 floats render nothing until their photos arrive.

If you genuinely want a visible placeholder while iterating LOCALLY (never committed live), it is an on-brand box: a gradient-frame card with a dark fill, an `IMAGE PLACEHOLDER` eyebrow label (in the page accent), and an italic muted description written like an AI image prompt (who is in it, what they are doing, the setting), floated or full-width to match where the real image goes. Swap the real `<img>` in (standard 2px gradient frame) before pushing.
