...
**Resources live on the Overview only (LOCKED):** every downloadable resource (preset, template, reference file, link) goes on the module Overview page. Step pages never repeat a resource download link; they reference it in prose and point back to THIS module's Overview (the first page of this module), so students never confuse it with the course Overview. One predictable place for resources builds an intuitive Canvas routine. The Overview's Downloads block is the canonical ORANGE section placed right after the intro/header card (2nd card), and it ends with the project-folder note (see the Downloads-section rules below).
