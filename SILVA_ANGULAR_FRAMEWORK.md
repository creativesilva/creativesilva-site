...
- Deliverables, per step (REQUIRED, no exception): each STEP is its own separately-graded Canvas submission with its own deliverable(s), always, unless Chris says otherwise (e.g. Sketchbook step 1 = upload 2 images, step 2 = upload the reflection: two graded assignments). The compact `DELIVERABLES` / `ENTREGABLES` box lives ONLY on the step page that submits that deliverable, placed LOW on that step (near the turn-in, not at the top), stating exactly what THAT step turns in with a clear file type (.docx, JPG, high-resolution JPG, .xmp). The box is **ORANGE-accented** (the same orange as Downloads, so turn-ins stand out from the teal content cards): `background:rgba(255,107,26,0.12);border:1px solid rgba(255,107,26,0.35);border-left:5px solid #FF6B1A`, eyebrow `#ffb27c` reading `DELIVERABLES &middot; TURN IT IN` / `ENTREGABLES &middot; ENTR&Eacute;GALO`, bullets `#FF6B1A`. NEVER put deliverables on the Overview (the Overview describes the project and holds downloads). EN in `#top`, ES in `#espanol`.
...
**Resources live on the Overview only (LOCKED):** every downloadable resource (preset, template, reference file, link) goes on the module Overview page. Step pages never repeat a resource download link; they reference it in prose and point back with a short line that names THIS module's Overview and the M dot, so students never confuse it with the course Overview: "on this module&rsquo;s Overview page (marked M at the top)." / "en la p&aacute;gina de Resumen de este m&oacute;dulo (marcada con M arriba)." One predictable place for resources builds an intuitive Canvas routine. The Overview's Downloads block is the canonical ORANGE section placed right after the intro/header card (2nd card), and it ends with the project-folder note (see the Downloads-section rules below).
