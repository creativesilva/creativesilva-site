...
- Assignment identification icon (REQUIRED where an icon exists): every active module page carries a small orange "assignment type" chip (`<!--IDCHIP-->`) right after the banner, icon + label, EN in `#top` and ES in `#espanol`. Icon family in `assets/Icons/assignment/`: `overview`, `your-device` (homework on the student's own phone/tablet), `camera-kit` (project needing a camera kit checkout), `photo-walk` (camera kit during class), `reflection`. Apply with the idempotent `tools/apply-id-icons.py` (add the page to its MAP with the type); re-run it after regenerating any builder module. Editing, contact-sheet, research/find, worksheet, and design/build steps have no icon yet, leave them off until Chris supplies one.
