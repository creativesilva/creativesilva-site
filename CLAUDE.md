...
11. **Deliverables callout (LOCKED 2026-08-31, REQUIRED, no exception). Lives ONLY on the step that submits it, placed LOW on that step, NEVER on the Overview.** Each STEP is its own separately-graded Canvas assignment with its own deliverable(s), ALWAYS unless Chris says otherwise. Example (Sketchbook Cover Art): step 1 = upload the 2 cover images; step 2 = upload the reflection: two separate graded assignments. The Deliverables callout belongs ONLY on the step page where that deliverable is submitted, stating EXACTLY what THAT step turns in (files, count, format). Do NOT put deliverables on the module Overview: the Overview describes the project and holds the downloadable resources; each step owns its own turn-in. Place the callout at the TOP of the step, right after the banner and before the content cards (LOCKED 2026-09-10, moved up from the old low placement, same as the Downloads section on Overviews), so students know upfront what they turn in for that step. Every step that submits work carries it (EN in `#top`, ES in `#espanol`); a step with genuinely nothing to submit is the exception, confirm with Chris. Use one consistent styled box titled `DELIVERABLES &middot; TURN IT IN` / `ENTREGABLES &middot; ENTR&Eacute;GALO`, **GOLD-accented (LOCKED 2026-09-10, its own color so turn-ins are unmistakable): `background:rgba(245,179,1,0.12);border:1px solid rgba(245,179,1,0.35);border-left:5px solid #f5b301`, eyebrow `#ffd166`, bullets `#f5b301`**, compact, with the **gold deliverables turn-in icon** (`assets/Icons/assignment/deliverables-v4.png`, 44px) on the right of the title row. Every deliverable states a clear file type (.docx, JPG, PDF, .xmp). When a builder generates a module, put the callout in the builder (the `deliverables_box` helper, emitted right after `banner`). NOTE the module accent system: **teal** = content, **orange** = Downloads, **purple `#8b5cf6`** = Resources, **gold `#f5b301`** = Deliverables.
12. **Contact sheets are ALWAYS turned in as high-resolution JPGs, never PDF.** The contact sheet preset already handles the format: it prints to file with the JPG setting preselected, so students never choose. NEVER write "JPG or PDF" (or "PDF or JPG") for a contact sheet in copy or deliverables, EN or ES: it is always "high-resolution JPG" / "JPG de alta resolución". Do not mention PDF for contact sheets at all.
