...
7b. **`assets/_incoming/` is a MOVE-through drop zone, not storage.** Chris drops raw images there for you to process. When you USE an image, file it OUT of `_incoming`: optimize + rename (versioned to bust cache) into the right `assets/images/<course>/<module>/` folder, wire it in, push, confirm live, THEN delete the raw original(s) from `_incoming`, including any superseded versions (e.g. remove both the v1 you replaced and the v2 raw once v2 is placed and live). Never leave used raws piling up: the incoming folder must not balloon with files to sort. The optimized asset under `assets/images/...` is the only retained copy. Full workflow in the [[image-intake-workflow]] memory.
