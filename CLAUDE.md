...
9. **Reflection documents (LOCKED 2026-08-31, polished header): text-box style + SEPARATE English and Spanish docs, never bilingual.** Every reflection/worksheet `.docx` uses this exact template: a square PV logo floating BEHIND the text top-left (`behindDocument:true`, wrap NONE), a CENTERED teal `#007474` title (~20pt), a CENTERED gray `#8A8A8A` subtitle line (`Course · Pioneer Valley High School · Mr. Silva`), a full-width teal rule, then a Name / Period / Date row. Each question is a full-width TEAL-FILLED bar (fill `007474`, white bold text) directly above its answer box; the answer box is a single bordered cell (border `C4C4C4`) holding a light-gray italic `Type your answer here.` placeholder, min-height ~1500 DXA, that grows as the student types. NEVER blank underlines for answers. And NEVER a single bilingual doc: build one complete English document AND one complete Spanish document, 100%, no shortcuts. On a bilingual module page, link the English doc in the English section and the Spanish doc in the Spanish section, and (per the overview-holds-resources rule) the download lives on the module Overview, not on a step page. Canonical builder: `tools/build-reflection.js` (one file builds all modules' EN + ES docs; the older `tools/build-*-reflection.js` are retired). The square logo asset is `assets/PV_Square_Logo.png`.
