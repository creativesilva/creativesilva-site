...
11. **Deliverables callout (LOCKED 2026-08-31, REQUIRED, no exception). Lives ONLY on the step that submits it, placed LOW on that step, NEVER on the Overview.** Each STEP is its own separately-graded Canvas assignment with its own deliverable(s), ALWAYS unless Chris says otherwise. Example (Sketchbook Cover Art): step 1 = upload the 2 cover images; step 2 = upload the reflection: two separate graded assignments. The Deliverables callout belongs ONLY on the step page where that deliverable is submitted, stating EXACTLY what THAT step turns in (files, count, format). Do NOT put deliverables on the module Overview: the Overview describes the project and holds the downloadable resources; each step owns its own turn-in. Place the callout LOW in the step, near the end at the turn-in point, NOT at the top under the banner. Every step that submits work carries it (EN in `#top`, ES in `#espanol`); a step with genuinely nothing to submit is the exception, confirm with Chris. Use one consistent styled box titled `DELIVERABLES` / `ENTREGABLES` (ORANGE-accented, compact, matching the Downloads section: `background:rgba(255,107,26,0.12);border:1px solid rgba(255,107,26,0.35);border-left:5px solid #FF6B1A`, eyebrow `#ffb27c`, bullets `#FF6B1A`). Orange so turn-ins stand out from the teal content cards. Every deliverable states a clear file type (.docx, JPG, PDF, .xmp). When a builder generates a module, put the callout in the builder.
