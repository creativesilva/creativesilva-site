...
11. **Deliverables callout (LOCKED 2026-08-31, REQUIRED, no exception). Each STEP is its own separately-graded submission.** Default model, ALWAYS unless Chris says otherwise: every step page is its own Canvas assignment that a student submits and that gets graded on its own, with its own deliverable(s). Example (Sketchbook Cover Art): step 1 = upload the 2 cover images; step 2 = upload the reflection: two separate graded assignments, NOT one combined turn-in. So the Deliverables callout on each step states EXACTLY what THAT step submits (files, count, format), and the module Overview summarizes the per-step submissions (a short "Step 1 turns in X, Step 2 turns in Y" list). Every step page carries the callout (EN in `#top`, ES in `#espanol`); a step with genuinely nothing to submit is the exception and must be confirmed with Chris. Use one consistent styled box titled `DELIVERABLES` / `ENTREGABLES` (teal-accented, compact, same component everywhere). Keep it tight, not bloated. When a builder generates a module, put the callout in the builder so every regenerated page keeps it.
