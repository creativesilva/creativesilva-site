...
1b. **No "shoot", "shooting", "shot" (or "screenshot").** Hard ban, as absolute as the em-dash ban. In a school setting these words carry the association of school shootings and are not acceptable. Applies everywhere: student copy, headings, chips, alt text, comments, commit messages, EN and ES. Use photography-safe words: shoot (verb) → capture / take photos / photograph; shooting → capturing / photographing; a shoot (session) → a session; shot / shots (a photo) → photo / image / frame; "shot in RAW" → "captured in RAW"; screenshot → screen capture. Before committing, grep `-iwE "shoot|shooting|shot|shots|shoots|screenshot"` and replace. (Filenames/URL slugs containing the substring are the only gray area; rename when practical.) **Manufacturer exception (Chris, 2026-09-11):** a maker's real menu or product name may use an otherwise-banned word when accuracy requires it, e.g. Canon's **Shooting** menu. Use it ONLY as the literal manufacturer term, never in your own prose. Builders that ban-scan output allowlist the exact phrases (e.g. "Shooting tab" / "Shooting menu").
...
5. **All student-facing assets hosted at `https://www.creativesilva.com/assets/...`** (the school-district-whitelisted domain; GitHub Pages serves `/assets/` at the site root). Do NOT use `raw.githubusercontent.com` or `cdn.jsdelivr.net`: the student Canvas iOS app cannot load images from non-whitelisted domains, so those hosts render as broken images in the app (they work in desktop browsers, which masks the problem). `creativesilva.com` is the ONLY domain the district approved, so it is the required asset host. Never use Canvas-hosted URLs. Never use relative paths in pages pasted into Canvas (the path won't resolve from inside Canvas). Verified 2026-08-25: images load in the student Canvas app only from `www.creativesilva.com`.
...
7b. **`assets/_incoming/` is a MOVE-through STAGING area, not storage. It must END EMPTY (only `README.md` stays).** Chris drops raw files there for you to process. When you USE a file, file it OUT of `_incoming`: optimize + rename (versioned to bust cache) into the right `assets/images/<course>/<module>/` folder, wire it in, push, confirm live, THEN delete the raw original(s) from `_incoming`, including any superseded versions (e.g. remove both the v1 you replaced and the v2 raw once v2 is placed and live). **Editable source masters you must keep (SVG / PSD / AI / vector originals) do NOT stay in `_incoming`: relocate them to a permanent `_src/` folder beside their output** (e.g. icon masters live in `assets/Icons/assignment/_src/`, not `_incoming`). Never leave used raws or masters piling up: `_incoming` must not balloon with files to sort. `_incoming` is gitignored except `README.md`, so its contents are local staging only. If you find files in `_incoming` you did NOT put there (student submissions, rosters, schedules, unrelated design source), do NOT delete them: surface them to Chris grouped, and move each to its proper home (or out of the repo) only on his say-so. The optimized asset under `assets/images/...` (plus any `_src/` master) is the only retained copy. Full workflow in the [[image-intake-workflow]] memory.
8. **No `.env` files, no credentials, no secrets.** Standard.
9. **Reflection documents (LOCKED 2026-08-31, polished header): text-box style + SEPARATE English and Spanish docs, never bilingual.** Every reflection/worksheet `.docx` uses this exact template: a square PV logo floating BEHIND the text top-left (`behindDocument:true`, wrap NONE), a CENTERED teal `#007474` title (~20pt), a CENTERED gray `#8A8A8A` subtitle line (`Course · Pioneer Valley High School · Mr. Silva`), a full-width teal rule, then a Name: / Period: / Date: row (bold teal labels, each ending in a colon). Each question is a full-width TEAL-FILLED bar (fill `007474`, white bold text) directly above its answer box; the answer box is a single bordered cell (border `C4C4C4`) holding a light-gray italic `Type your answer here.` placeholder, min-height ~1500 DXA, that grows as the student types. NEVER blank underlines for answers. And NEVER a single bilingual doc: build one complete English document AND one complete Spanish document, 100%, no shortcuts. On a bilingual module page, link the English doc in the English section and the Spanish doc in the Spanish section, and (per the overview-holds-resources rule) the download lives on the module Overview, not on a step page. Canonical builder: `tools/build-reflection.js` (one file builds all modules' EN + ES docs; the older `tools/build-*-reflection.js` are retired). The Brand Color Analysis worksheet (Digital Arts color theory) has its own builder `tools/build-color-worksheet.js` (separate EN + ES, same header, image-paste + touchpoint layout); it too is never bilingual. The square logo asset is `assets/PV_Square_Logo.png`.
10. **OWN-DEVICE photo capture assignments carry an honesty/integrity notice.** When students shoot on their OWN DEVICE (phone or personal camera), add a clear orange note (EN + ES) on the capture step AND the overview: they must take FRESH photos, planned and captured on purpose for that assignment, and may NOT reuse pictures already in their camera roll from before the class. Canonical wording: &#x201C;Fresh photos only. Do not use pictures already in your camera roll from before this class. Every photo must be planned and taken on purpose for this project. Be honest and turn in your own new work.&#x201D; / &#x201C;Solo fotos nuevas. No uses fotos que ya ten&iacute;as en tu galer&iacute;a de antes de esta clase. Cada foto debe ser planeada y tomada a prop&oacute;sito para este proyecto. S&eacute; honesto y entrega tu propio trabajo nuevo.&#x201D; **This applies ONLY to own-device assignments (LOCKED 2026-09-10).** When students use a SCHOOL CAMERA KIT they are inherently making new images with class gear (no personal camera roll to mine), so the note is NOT used and should not appear. Own-device modules: Self-Portrait, Composition Concepts. Camera-kit modules (Image Series, Leading Lines, Photo 2 Composition, OCF, Build Your Own Preset, Studio Session) do not carry it.
11. **Deliverables callout (REQUIRED, no exception). Lives ONLY on the step that submits it, placed right AFTER the step's opening teal information section (the step LEADS with teal, never with the gold box), NEVER on the Overview.** Each STEP is its own separately-graded Canvas assignment with its own deliverable(s), ALWAYS unless Chris says otherwise. Example (Sketchbook Cover Art): step 1 = upload the 2 cover images; step 2 = upload the reflection: two separate graded assignments. The Deliverables callout belongs ONLY on the step page where that deliverable is submitted, stating EXACTLY what THAT step turns in (files, count, format). Do NOT put deliverables on the module Overview: the Overview describes the project and holds the downloadable resources; each step owns its own turn-in. Lead the step with the teal information section (its intro content), then place the callout right AFTER that first teal section, never first (LOCKED 2026-09-24, Chris: "I wanted the teal information verbiage section first, never lead with the yellow." This SUPERSEDES the earlier 2026-09-10 "top of the step" placement). Every step that submits work carries it (EN in `#top`, ES in `#espanol`); a step with genuinely nothing to submit is the exception, confirm with Chris. Use one consistent styled box titled `DELIVERABLES &middot; TURN IT IN` / `ENTREGABLES &middot; ENTR&Eacute;GALO`, **GOLD-accented (LOCKED 2026-09-10, its own color so turn-ins are unmistakable): `background:rgba(245,179,1,0.12);border:1px solid rgba(245,179,1,0.35);border-left:5px solid #f5b301`, eyebrow `#ffd166`, bullets `#f5b301`**, compact, with the **gold deliverables turn-in icon** (`assets/Icons/assignment/deliverables-v4.png`, 44px) on the right of the title row. Every deliverable states a clear file type (.docx, JPG, PDF, .xmp). When a builder generates a module, put the callout in the builder (the `deliverables_box` helper, emitted right AFTER the step's opening teal information section, NOT right after `banner`). NOTE the module accent system: **teal** = content, **orange** = Downloads, **purple `#8b5cf6`** = Resources, **gold `#f5b301`** = Deliverables.
12. **Contact sheets are ALWAYS turned in as high-resolution JPGs, never PDF.** The contact sheet preset already handles the format: it prints to file with the JPG setting preselected, so students never choose. NEVER write "JPG or PDF" (or "PDF or JPG") for a contact sheet in copy or deliverables, EN or ES: it is always "high-resolution JPG" / "JPG de alta resolución". Do not mention PDF for contact sheets at all. **Do NOT bundle the contact-sheet template ZIP in a module's Downloads (LOCKED 2026-09-24, Chris): once installed in Lightroom the presets persist until deleted, so students already have them; the templates live on the Course Overview if anyone needs them again. A module's Downloads carries only that module's own files (e.g. its reflection doc).** **The RAW/JPG capture format is shown ONLY inside the red camera-settings section (its Image Quality cell), NEVER as a standalone `capture_format` chip on the step (LOCKED 2026-09-24).**
13. **Mac platform only, never PC/Windows.** All how-to steps are for macOS (the classroom runs on Macs). Use Cmd (not Ctrl), Option (not Alt), Finder, Control-click, the top menu bar. Do NOT add Windows alternates or PC paths. Applies to EN and ES student copy, staff notes, and slide decks.
14. **Never call chosen/culled images "picks."** When students cull or select images (the strongest photos, the final set, edited favorites), call them **selections**, **image selections**, **selected images**, or **culled images**. NEVER the noun "picks" (e.g. "your edited picks" &rarr; "your edited selections"). Prefer the verb **select** or **cull** over "pick" when choosing images ("select your best 6", not "pick your best 6"). This is ONLY about culling/selecting images: "pick" is fine for non-image choices (a tool, a font/typeface, a camera setting or mode, a concept/idea). Applies to EN and ES; the Spanish already uses seleccionar/elegir.
15. **The person being photographed is the "subject," never "talent" or "model" (LOCKED 2026-09-24).** In any partner or portrait module, call the person in front of the camera the **subject** (EN) / **sujeto** (ES). NEVER "talent" or "model" (EN), NEVER "modelo" or "talento" (ES): those frame the role as a performance and make camera-shy students feel they must perform. "Subject" is the neutral photography term (the person being photographed). Applies to titles, body copy, and captions across ALL modules, EN and ES; keep the pairing "Photographer and Subject" / "Fot&oacute;grafo y Sujeto." Do NOT add any explanation about shyness or performing into student copy: just use the neutral word.
